...
function(configure_pkg_config_file_vars
    ...
    if(NOT _TARGET_TYPE IN_LIST _SUPPORTED_TARGET_TYPES)
        message(FATAL_ERROR
            "configure_pkg_config_file does not support ${_TARGET_TYPE} "
            "target '${TARGET}'")
    endif()
    ...
    if("${_DESCRIPTION}" STREQUAL "")
        message(FATAL_ERROR
            "configure_pkg_config_file requires DESCRIPTION")
    endif()
    if("${_VERSION}" STREQUAL "")
        message(FATAL_ERROR
            "configure_pkg_config_file requires VERSION")
    endif()
    ...
    if(IS_ABSOLUTE "${_INSTALL_INCLUDE_DIR}"
       OR IS_ABSOLUTE "${_INSTALL_LIB_DIR}")
        message(FATAL_ERROR
    ...
    set(_PKG_CONFIG_OUTPUT_DIR
        "${CMAKE_CURRENT_BINARY_DIR}/pkgconfig/$<CONFIG>")
    set(_PKG_CONFIG_FILE
        "${_PKG_CONFIG_OUTPUT_DIR}/${_NAME}.pc")
    ...
    if(NOT _IS_INTERFACE)
        string(APPEND _CONTENT
            "libdir=\${prefix}/${_INSTALL_LIB_DIR}\n")
    endif()
    if(_HAS_PUBLIC_INCLUDES)
        string(APPEND _CONTENT
            "includedir=\${prefix}/${_INSTALL_INCLUDE_DIR}\n")
    endif()
    ...
    if(_IS_OBJECT)
        string(MD5 _TARGET_ID
            "${CMAKE_CURRENT_BINARY_DIR};${TARGET};${_NAME}")
        set(_OBJECTS_PLACEHOLDER
            "__GEN_PKG_CONFIG_OBJECTS_${_TARGET_ID}__")
        set(_TARGET_OBJECTS_FILE
            "${_PKG_CONFIG_OUTPUT_DIR}/${_NAME}.objects")
        set(_PKG_CONFIG_FILE_UNFINISHED
            "${_PKG_CONFIG_FILE}.unfinished")
        file(GENERATE
