...
foreach(object_path IN LISTS TARGET_OBJECTS)
    ...
    if(object_name_key IN_LIST OBJECT_NAME_KEYS)
        message(FATAL_ERROR
            "Object files for pkg-config must have unique basenames: "
            "${object_name}")
    endif()
    list(APPEND OBJECT_NAME_KEYS "${object_name_key}")
    string(REPLACE "\\" "\\\\" object_name "${object_name}")
    string(REPLACE " " "\\ " object_name "${object_name}")
    list(APPEND PROPERLY_JOINED_TARGET_OBJECTS
        "\${libdir}/${object_name}")
endforeach()
...
string(FIND "${PKG_CONFIG_CONTENT}" "${objectsPlaceholder}"
    OBJECTS_PLACEHOLDER_INDEX)
if(OBJECTS_PLACEHOLDER_INDEX EQUAL -1)
    message(FATAL_ERROR
        "Object placeholder is missing from the unfinished pkg-config file")
endif()
string(REPLACE "${objectsPlaceholder}" "${PROPERLY_JOINED_TARGET_OBJECTS}"
    PKG_CONFIG_CONTENT "${PKG_CONFIG_CONTENT}")
